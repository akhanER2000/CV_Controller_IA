...
**Estado: pendiente de verificación real.** La extracción de HTML (ver `cv-texto-plano.txt`)
confirma orden de lectura y espaciado de marcado, pero **no** los *runs* de fuente — que solo
existen en el PDF. **Contrato CI:** re-parsear el PDF de `react-pdf` contra `cv-texto-plano.txt`
(§7); mientras tanto, el mono es una apuesta mitigada por las 5 reglas de arriba.

### Plan B (escrito, para no improvisarlo)

Si el re-parseo del PDF real muestra que el run mono se pega o se separa **y** las reglas 1–4 no lo
resuelven:

- La cifra **vuelve a Geist** con `font-weight: 600` dentro de la viñeta (misma familia que el
  cuerpo → un solo run, cero riesgo de separación).
- Se **pierde el mono en el documento**; se **conserva en la app** (preview, "cómo lo lee el ATS",
  métricas del dashboard), donde no hay parser que respetar.
- Es una degradación de un solo token (`--cv-num-font`), sin tocar layout ni jerarquía. La cifra
  sigue destacando por peso.
- No es una derrota: es la evidencia mandando sobre el gusto, exactamente como promete el producto.
