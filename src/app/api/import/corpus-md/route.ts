...
/* ============================================================================
   ...
   Cuerpo:
     { text }                        el .md pegado / leído por el navegador
     { path, name? }                 referencia a Storage (bucket 'sources'), como /api/sources
...
export async function POST(req: Request) {
  ...
  const soloAnalizar = body.analizar === true;
  const lang = typeof body.lang === "string" && body.lang.trim() ? body.lang.trim() : "es";
  const path = typeof body.path === "string" ? body.path : "";
  let nombre = typeof body.name === "string" && body.name.trim() ? body.name.trim() : "";
  let texto = typeof body.text === "string" ? body.text : "";
  let storagePath: string | null = null;

  try {
    // ── El fichero: o viene en el cuerpo, o se baja de Storage (igual que /api/sources) ──
    if (!texto && path) {
      ...
      if (error || !blob) {
        return NextResponse.json(
          { error: `No se pudo leer el archivo de Storage: ${error?.message ?? "sin contenido"}` },
          { status: 422 },
        );
      }
